...
"""
dorori_obd.py
DORORI 프로젝트: 실제 카메라/YOLO/모터 + USB ELM327 OBD-II 속도 서버.
...
RUN_MODE = "OBD"
HTML_FILE_NAME = "dorori_obd.html"
DOOR_ID = "RR"
...
MOTOR_STEPS_PER_REV = int(os.environ.get("MOTOR_STEPS_PER_REV", "1600"))
...
OBD_BAUDRATE = env_optional_int("OBD_BAUDRATE")
OBD_FAST = env_bool("OBD_FAST", True)
OBD_TIMEOUT_SEC = float(os.environ.get("OBD_TIMEOUT_SEC", "0.2"))
OBD_POLL_SEC = float(os.environ.get("OBD_POLL_SEC", "0.2"))
OBD_STALE_SEC = float(os.environ.get("OBD_STALE_SEC", "1.5"))
OBD_RETRY_SEC = float(os.environ.get("OBD_RETRY_SEC", "2.0"))
...
def speed_is_fresh(vehicle: dict[str, Any] | None = None) -> bool:
    ...
    if not vehicle.get("connected") or vehicle.get("speed_kmh") is None:
        return False
    updated_at = float(vehicle.get("updated_at") or 0.0)
    if updated_at <= 0.0:
        return False
...
def obd_loop() -> None:
    while not shutdown_event.is_set():
        ...
        try:
            if obd is None:
                raise RuntimeError("python-OBD 패키지가 없습니다. 'pip install obd'를 실행하세요.")

            print(f"[OBD] 연결 시도: {OBD_PORT or 'AUTO'}")
            kwargs: dict[str, Any] = {
                "fast": OBD_FAST,
                "timeout": OBD_TIMEOUT_SEC,
            }
            if OBD_BAUDRATE is not None:
                kwargs["baudrate"] = OBD_BAUDRATE

            connection = obd.OBD(OBD_PORT, **kwargs)
            if not connection.is_connected():
                raise RuntimeError(f"OBD 차량 연결 실패: {connection.status()}")

            status_text = str(connection.status())
            with vehicle_lock:
            ...
            print(f"[OBD] 연결 완료: {status_text}")

            null_count = 0
            while not shutdown_event.is_set():
                # 일부 차량/복제 ELM327은 지원 PID 목록 판별이 부정확할 수 있어
                # 표준 Vehicle Speed PID(01 0D)를 force=True로 직접 질의합니다.
                ...
                if response.is_null() or response.value is None:
                    ...
                    shutdown_event.wait(OBD_POLL_SEC)
                    continue
        ...
        shutdown_event.wait(OBD_RETRY_SEC)
...
class MotorController:
    ...
    def _initialize_gpio(self) -> None:
        ...
        try:
            ...
            print(
                ...
                f"ENABLE={ENABLE_PIN}, steps/rev={MOTOR_STEPS_PER_REV}, "
    ...
    def _cycle_worker(self, cycle_id: int, source: str) -> None:
        try:
            ...
            # 정상 전개라면 한 바퀴, 조기 중단이라면 실제 전개된 펄스 수만큼 역회전합니다.
            self._update_state(
                state="RETRACTING",
            ...
            if not motor_state["ready"]:
                ...
                if ok:
                    update_system(
                        ...
                        reason_text="발판 1회전 전개 후 5초 유지, 자동 수납을 시작했습니다.",
                    )
...
def main() -> None:
    ...
    threads = [
        ...
        threading.Thread(target=obd_loop, name="obd-loop", daemon=True),
    ]
